...
let arr = ref([]);
const governanceListEn = reactive([
  {
    id: 0,
    acode: "cn",
    title: "",
...
async function fetchData() {
  await fetch("https://cmereye.com/backend/api.php/list/27")
    ...
    .then((data) => {
      // 清空数组
      governanceListEn.splice(0, governanceListEn.length);
      arr.value = data.data;
      arr.value.map((item: any) => {
        governanceListEn.push({
          id: item.id,
          acode: item.acode,
          title: item.title,
...
const pageData = (governanceListEn: any[]) => {
  governanceListEn.forEach((item: any, index: number) => {
    item.ico = imgList[index];
  });
  if (governanceListEn.length % 2 !== 0) {
    // 在数组的第二个插入空
    governanceListEn.splice(1, 0, { id: 0, acode: "", title: "", ico: "" });
  }
};
onMounted(async () => {
  await fetchData();
...
<template>
  <div class="governance">
    <div class="governance-list">
      <div v-for="item in governanceListEn" :key="item.id">
        ...
        <div v-if="item.title !== ''">
          ...
          <a
            ...
            >{{ item.title.length <= 0 ? "" : "VIEW NOW" }}</a
          >
        </div>
      </div>
    </div>
